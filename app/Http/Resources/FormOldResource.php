...
class FormOldResource extends BaseResource
{
    ...
    {
        $submissions = Submission::where('location_id', $request->data["location_id"])->where('form_group_id', $this->id)->latest()->first();
        ...
        foreach ($latest->forms as $f_key => $f_val) {
            ...
            foreach ($f_val->formAttributes as $key => $val) {
                ...
                foreach ($val['locations'] as $location) {
                    ...
                    $location_obj['value'] = $oldest_data["input_value"];
